...
class Model(torch.nn.Module):
    def __init__(self, data, args):
        ...
        if args.model == 'ours':
            self.model = LSTC(args)

        else:
            raise ValueError(f"Unsupported model type: {args.model}")

    def forward(self, timeIdx, userIdx, servIdx, contrastive=False):
        y = self.model(timeIdx, userIdx, servIdx, contrastive)
        return y
    ...
    def train_one_epoch(self, dataModule):
        ...
        for train_Batch in (dataModule.train_loader):
            ...
            timeIdx, userIdx, servIdx, value = tuple(item.to(self.args.device) for item in train_Batch)
            pred, contrastive_loss = self.forward(timeIdx, userIdx, servIdx, contrastive=True)
            loss = self.loss_function(pred, value) + 0.001 * contrastive_loss
    ...
    def evaluate_one_epoch(self, dataModule, mode='valid'):
        ...
        for batch in (dataloader):
            timeIdx, userIdx, servIdx, value = tuple(item.to(self.args.device) for item in batch)
            pred = self.forward(timeIdx, userIdx, servIdx)
            if mode == 'valid':
...
def RunOnce(args, runId, log):
    ...
    if retrain_required:
        ...
        for epoch in trange(args.epochs):
            ...
            monitor.track_one_epoch(epoch, model, valid_error, 'NRMSE')
...
if __name__ == '__main__':
    ...
    log_filename = f'Model_{args.model}_{args.dataset}_D{args.density:.3f}_R{args.rank}'
