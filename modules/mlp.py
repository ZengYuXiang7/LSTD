...
class MLPTimeRefiner(Module):
    def __init__(self, args):
        ...
        self._time_linear = Linear(args.rank, args.rank)
        self.context = Sequential(
            Linear(self.num_windows * args.rank, args.rank),
            LayerNorm(args.rank),
            ReLU(),
            Linear(args.rank, args.rank)
        )

        self.rainbow = torch.arange(-self.num_windows + 1, 1).reshape(1, -1).to(args.device)
        self.attn = Sequential(Linear(2 * args.rank, 1), Tanh())

    def forward(self, time_embeds):
        bs, num_windows, rank = time_embeds.size()

        # attention pred_layer
